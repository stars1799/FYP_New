...
```{r}
amk_ep_route <- tm_shape(amk_res_block) +
...
# Adding the linestring from amk_res_ww
amk_ep_route <- amk_ep_route +
  tm_shape(amk_res_ww) +
  tm_lines(lwd = 1, col = "red")

# Adding the linestring from amk_res_shortcut
amk_ep_route <- amk_ep_route +
  tm_shape(amk_res_s1) +
...
# Adding the geometry points from amk_res_pge
amk_ep_route <- amk_ep_route +
...
```{r}
pg_ep_route <- tm_shape(pg_res_block) +
...
pg_ep_route <- pg_ep_route +
  tm_shape(pg_res_ww) +
  tm_lines(lwd = 1, col = "red")

# Adding the linestring from pg_res_shortcut
pg_ep_route <- pg_ep_route +
  tm_shape(pg_res_s1) +
  tm_lines(lwd = 1, col = "green")
pg_ep_route <- pg_ep_route +
...
```{r}
pg_sa_route <- tm_shape(pg_sa_res) +
  tm_symbols(shape = housesym, size = 0.1, border.lwd = NA) +  # Plot geometry points with red color and smaller size
  tm_layout(legend.show = TRUE) +  # Show legend
  osm_basemap  # Add basemap

pg_sa_route <- pg_sa_route +
  tm_shape(pg_sa_ww) +
  tm_lines(lwd = 1, col = "red")
...
# Read data
data <- read.csv('data/aspatial/combined_data.csv')
```
...
set.seed(123)
res.km <- kmeans(data[, -5], 3, nstart = 25)
...
hc_data<- data.frame(
row.names = c("A", "B", "C"),
time_if_the_day = c(0.453333, 0.302041, 0.707143),
weather = c(0.376500, 0.600204, 0.585714),
safety = c(0.275000, 0.605442, 0.666667),
terrain_elevation = c(0.150000, 0.530612, 0.550000)
)
hc_data
```
...
```{r}
library(tidyverse) 
library(ggradar)  # Load the package without quotes
hc_df <- hc_data %>% rownames_to_column("group")
hc_df
```
...
sc_data1<- data.frame(
row.names = c("A", "B", "C"),
time_if_the_day = c(0.628049, 0.117647, 0.250000),
weather = c(0.653621, 0.669707, 0.508183),
safety = c(0.634146, 0.647059, 0.358730),
terrain_elevation = c(0.723577, 0.777778, 0.407407)
)
sc_data1
```
...
library(tidyverse) 
library(ggradar)  # Load the package without quotes
sc_df1 <- sc_data1 %>% rownames_to_column("group")
sc_df1
```
...
sc_data2<- data.frame(
row.names = c("A", "B", "C"),
time_if_the_day = c(0.316667, 0.127551, 0.633929),
weather = c(0.516667, 0.690081, 0.678848),
safety = c(0.378571, 0.661808, 0.714286),
terrain_elevation = c(0.458333, 0.775510, 0.791667)
)
sc_data2
```
...
library(tidyverse) 
library(ggradar)  # Load the package without quotes
sc_df2 <- sc_data2 %>% rownames_to_column("group")
sc_df2
```
